...
__global__ void cmp_win(oArray *output, const iArray *input)
{
    ...
    if ((idx < DATAHSIZE+WSIZE) && (idy < DATAWSIZE+WSIZE))
      smem[threadIdx.x][threadIdx.y]=input[idx][idy];
    if ((idx < DATAHSIZE+WSIZE) && (idy < DATAWSIZE) && (threadIdx.y > BLKWSIZE - WSIZE))
        smem[threadIdx.x][threadIdx.y + (WSIZE-1)] = input[idx][idy+(WSIZE-1)];
    if ((idx < DATAHSIZE) && (idy < DATAWSIZE+WSIZE) && (threadIdx.x > BLKHSIZE - WSIZE))
        smem[threadIdx.x + (WSIZE-1)][threadIdx.y] = input[idx+(WSIZE-1)][idy];
    if ((idx < DATAHSIZE) && (idy < DATAWSIZE) && (threadIdx.x > BLKHSIZE - WSIZE) && (threadIdx.y > BLKWSIZE - WSIZE))
        smem[threadIdx.x + (WSIZE-1)][threadIdx.y + (WSIZE-1)] = input[idx+(WSIZE-1)][idy+(WSIZE-1)];
    ...
    if ((idx < DATAHSIZE) && (idy < DATAWSIZE)){
      ...
      for (i=0; i<WSIZE; i++)
        for (j=0; j<WSIZE; j++)
          if (smem[threadIdx.x + i][threadIdx.y + j] < tempout)
            tempout = smem[threadIdx.x + i][threadIdx.y + j];
